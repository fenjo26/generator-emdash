...
Кастомная страница ошибки генерируется автоматически — никаких дополнительных настроек не нужно.

**Что входит в сборку:**

- `dist/404.html` — страница в стиле сайта: большой градиентный «404», текст на греческом, кнопка «← Επιστροφή στην Αρχική». Помечена `noindex` — гугл не штрафует за её существование.
- `dist/.htaccess` — Apache подхватывает автоматически после деплоя:
  ```apache
  ErrorDocument 404 /404.html
  ```
- `nginx-404.conf` — генерируется в корне output-папки как готовый сниппет для Nginx.

**Настройка Nginx в Hestia CP (один раз на домен):**

1. Hestia CP → Web → домен → Edit → поле **Nginx Config**
2. Вставить содержимое `nginx-404.conf`:

```nginx
error_page 404 /404.html;
location = /404.html {
    root /home/work/web/DOMAIN/public_html;
    internal;
}
```

`internal` означает что браузер получает настоящий HTTP 404, а страница `/404.html` недоступна напрямую по URL — это правильно для SEO.

> **Примечание:** Если Hestia работает на связке Nginx + Apache, достаточно только `.htaccess` — Apache обрабатывает 404 раньше. Nginx-сниппет нужен только при чистом Nginx без Apache.
